<?php 
    ...
    // courtesy to https://github.com/hugomd/ascii-live/blob/master/frames/rick.go
    $rickFrames = [
...
11111111111111111i,,i:..........,:,,,.,,.,.......::,,i,it1ttt111111111\n"
    ...
    if(isset($_SERVER['HTTP_ACCEPT']) && 
            strpos($_SERVER['HTTP_ACCEPT'], 'text/html') !== false &&  // Common in browsers
            isset($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
        echo "Please use curl to see magikk!!!";
        exit();
    } else {
        header("Content-Type: plain/text");
        $it = 0;
        $ms = 60000;
        // echo count($rickFrames);
        while(true) {
            if($it > (count($rickFrames) - 1)) {
                $it = 0;
                // usleep($ms);
                print($rickFrames[$it]);
                continue;
